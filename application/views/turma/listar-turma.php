...
<script type="text/javascript">
    function excluir(id, linha) {
        var url = "<?php echo base_url() ?>turma/excluir/";
        if (confirm("Deseja realmente excluir esse turma? \n Todas matriculas referentes ficarão inconsistentes")) {
                ...
                <table id="sample-table-2" class="table table-striped table-bordered table-hover">
                    ...
                    <tbody>
                        ...
                        if (!isset($turmas) || empty($turmas)) {
                            echo '<div class="alert alert-danger">Nenhum turma cadastrada</div>';
                        } else {
                            foreach ($turmas as $t) {
                                ?> 	
                                <tr id="linha<?php echo $numLinha; ?>">
                                    ...
                                    <td><?php echo $t->id_turma; ?> </td>
                                    <td> <?php echo $t->nome_turma; ?>  </td>                                   
                                    <td> <?php echo $t->nome_curso; ?>  </td> 
                                    <td>
                                        <div class="visible-md visible-lg hidden-sm hidden-xs action-buttons">

                                            <a class="green" title="editar infomações" href="<?php echo base_url().'turma/editar/'.$t->id_turma.'/'.url_title($t->nome_turma);?>">
                                                <i class="icon-pencil bigger-130"></i>
                                            </a>

                                            <a class="red" title="apagar" href="" onClick="return excluir(<?php echo $t->id_turma, ', ', $numLinha; ?>);">
