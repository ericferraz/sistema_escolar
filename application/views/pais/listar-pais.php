...
<script type="text/javascript">
    function excluir(id, linha) {
        var url = "<?php echo base_url() ?>pais/excluir/";
        if (confirm("Deseja realmente excluir este país? \n todos estados referentes a esse país serão apagados")) {
                ...
                <table id="sample-table-2" class="table table-striped table-bordered table-hover">
                    ...
                    <tbody>
                        ...
                        if (!isset($paises) || empty($paises)) {
                            echo '<div class="alert alert-danger">Nenhum país cadastrado</div>';
                        } else {
                            foreach ($paises as $p) {
                                ?> 	
                                <tr id="linha<?php echo $numLinha; ?>">
                                    ...
                                    <td> <?php echo $p->nome_pais; ?>  </td>                                   
                                  
                                    <td>
                                        <div class="visible-md visible-lg hidden-sm hidden-xs action-buttons">

                                            <a class="green" title="editar infomações" href="<?php echo base_url().'pais/editar/'.$p->id_pais.'/'.url_title($p->nome_pais);?>">
                                                <i class="icon-pencil bigger-130"></i>
                                            </a>

                                            <a class="red" title="apagar" href="" onClick="return excluir(<?php echo $p->id_pais, ', ', $numLinha; ?>);">
